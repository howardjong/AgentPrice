...
describe('jobManager Integration Tests', () => {
  ...
  test('should apply rate limiting for deep research when specified', async () => {
    ...
    // Verify some information was logged - we can't check exact message content
  ...
  test('should maintain consistent interface between real and mock implementations', async () => {
    ...
    // We'll skip the direct comparison of keys and just verify that both implementations
    // have the expected methods (this is more flexible as implementations evolve)
    const requiredMethods = ['enqueueJob', 'getJobStatus', 'registerProcessor'];
    
    requiredMethods.forEach(method => {
      // Check that both implementations have the required methods,
      // but don't force them to be identical overall
