...
class CostTracker {
   ...
   */
  getStatus() {
    const stats = this.getStats({ detailed: true });
    const now = new Date();
    const currentHour = now.getHours();
    const dayKey = `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}`;
    const monthKey = `${now.getFullYear()}-${now.getMonth() + 1}`;
    
    // Calculate active cost saving features
    const activeFeatures = Object.entries(this.costSavingFeatures)
      .filter(([_, feature]) => feature.enabled)
      .map(([name, _]) => name);
    
    // Calculate today's cost
    const todayCost = this.costs.daily[dayKey]?.cost || 0;
    
    // Calculate month-to-date cost
    const monthToDateCost = this.costs.monthly[monthKey]?.cost || 0;
    ...
    return {
      ...
      costTracking: {
        ...
        monthToDateCost: parseFloat(monthToDateCost.toFixed(4)),
      ...
      savingFeatures: {
        activeFeatures,
        savings: {
          caching: parseFloat(this.costSavingFeatures.caching.savings.toFixed(4)),
          tokenOptimization: parseFloat(this.costSavingFeatures.tokenOptimization.savings.toFixed(4)),
          modelDowngrading: parseFloat(this.costSavingFeatures.modelDowngrading.savings.toFixed(4)),
          batchProcessing: parseFloat(this.costSavingFeatures.batchProcessing.savings.toFixed(4)),
          apiDisabling: parseFloat(this.costSavingFeatures.apiDisabling.savings.toFixed(4))
        }
      },
      serviceBreakdown: Object.entries(this.costs.services).reduce((acc, [service, data]) => {
        acc[service] = {
          calls: data.calls,
