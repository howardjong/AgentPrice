  ...
  async performDeepResearch(query, options = {}) {
    ...
    if (areLlmCallsDisabled()) {
      ...
      return {
        ...
        content: `[Using Perplexity AI - Model: ${this.models.deepResearch} (MOCK)]\n\nThis is a mock response because LLM API calls are disabled. Enable API calls to get real results.`,
        sources: [],
        modelUsed: `${this.models.deepResearch} (MOCK)`,
    ...
    return perplexityRateLimiter.schedule(async () => {
      try {
        logger.info(`Starting deep research with Perplexity for query: "${query.substring(0, 50)}${query.length > 50 ? '...' : ''}"`, {
          service: 'multi-llm-research',
          modelRequested: this.models.deepResearch
        });
...
Please include the most up-to-date information available, with particular attention to recent developments and reliable sources. Format your response in a clear, organized manner with appropriate headings and citation references where applicable.`;
  ...
  async performChunkedDeepResearch(query, options = {}) {
    ...
    try {
      ...
      return {
        ...
        sources: allSources,
        modelUsed: chunkResponses[0]?.modelUsed || this.models.deepResearch,
        requestedModel: this.models.deepResearch,
        usage: totalUsage,
  ...
  determineModelForQuery(query, options = {}) {
    ...
    if (options.model) {
      // Check if the requested model is a legacy model name
      if (this.legacyModelMapping[options.model]) {
        const mappedModel = this.legacyModelMapping[options.model];
        logger.info(`Mapping legacy model ${options.model} to ${mappedModel}`, {
    ...
    if (options.deepResearch === true) {
      logger.info(`Using deep research model (${this.models.deepResearch}) - explicitly requested`, {
        queryLength: query.length
      });
      return this.models.deepResearch;
    }

    // Always default to the basic model unless explicitly requested otherwise
    logger.info(`Using default basic model (${this.models.basic})`, {
      queryLength: query.length
    });
