/**
 * Simple Perplexity Deep Research Model Test
 * 
 * This script tests the Perplexity API with the sonar-deep-research model
 * and high search context setting.
 */
...
async function executeQuery(query, options = {}) {
  ...
  log(`Querying Perplexity with model ${model} (searchContext: ${searchContext}): "${query.substring(0, 100)}${query.length > 100 ? '...' : ''}"`);
...
async function runTest() {
  log('=== Starting Simple Perplexity Deep Research Model Test ===');
  ...
  try {
    ...
    const outputFile = path.join(OUTPUT_DIR, `deep-research-model-test-${timestamp}.json`);
    ...
    log('=== Deep Research Model Test Completed Successfully ===');
